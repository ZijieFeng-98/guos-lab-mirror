...
# =============================================================================
# Load and Combine GSE222520 Expression Matrices (Fixed)
# Load 10x Genomics format data with correct directory structure
# =============================================================================
...
# Function to find 10x data directories
find_10x_directories <- function(base_dir) {
    # Look for filtered_feature_bc_matrix directories
...
for (i in seq_along(extracted_samples)) {
    ...
    # Find 10x data directories
    data_dirs <- find_10x_directories(sample_dir)
    
    if (length(data_dirs) == 0) {
        cat("  No 10x data directories found\n")
        next
    }
    
    cat("  Found", length(data_dirs), "10x data directories\n")
    
    for (data_dir in data_dirs) {
        tryCatch({
            # Extract sample name from path
            path_parts <- strsplit(data_dir, "/")[[1]]
            sample_name <- path_parts[length(path_parts) - 1]  # The directory before filtered_feature_bc_matrix
            
            # Read the 10x data
            data <- read_10x_data(data_dir)
            
            # Add sample prefix to cell barcodes to avoid conflicts
            colnames(data$matrix) <- paste0(sample_name, "_", colnames(data$matrix))
            ...
            sample_info <- rbind(sample_info, data.frame(
                sample = sample_name,
                sample_type = gsub(".*_", "", sample_name),
            ...
            cat("  Error loading data from", data_dir, ":", e$message, "\n")
        })
    }
}
...
save_script_info(
    script_name = "load_and_combine_matrices_fixed.R",
    description = "Load and combine 10x Genomics expression matrices with correct directory structure",
