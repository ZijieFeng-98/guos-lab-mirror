# APOE Analysis in Single-Cell RNA-seq Data
# GSE222520 - Brain Tumor Leukocyte Single-Cell Analysis
# Focus on APOE expression patterns and isoform analysis
...
extract_sample_info <- function(metadata_file) {
  ...
  # Check which columns exist and handle missing ones
  col_names <- colnames(metadata)
  
  # Extract key information with safe column access
  sample_info <- data.frame(
    sample_id = if("title" %in% col_names) metadata$title else rep(NA, nrow(metadata)),
    geo_accession = if("geo_accession" %in% col_names) metadata$geo_accession else rep(NA, nrow(metadata)),
    tissue = if("tissue:ch1" %in% col_names) metadata$`tissue:ch1` else rep(NA, nrow(metadata)),
    diagnosis = if("diagnosis:ch1" %in% col_names) metadata$`diagnosis:ch1` else rep(NA, nrow(metadata)),
    genotype = if("genotype:ch1" %in% col_names) metadata$`genotype:ch1` else rep(NA, nrow(metadata)),
    primary_recurrent = if("primary/recurrent:ch1" %in% col_names) metadata$`primary/recurrent:ch1` else rep(NA, nrow(metadata)),
    treatment = if("treatment:ch1" %in% col_names) metadata$`treatment:ch1` else rep(NA, nrow(metadata)),
    cell_type = if("cell type:ch1" %in% col_names) metadata$`cell type:ch1` else rep(NA, nrow(metadata)),
...
# Define sample groups (similar to APOE genotype logic)
...
# Add APOE-related grouping based on genotype
...
# APOE Analysis - Focus on expression patterns
cat("\n=== APOE EXPRESSION ANALYSIS ===\n")
...
if (length(apoe_results) > 0) {
  ...
  all_apoe_stats$apoe_group <- sample_meta$apoe_group
  ...
  # Create visualizations similar to genotype analysis
  cat("Creating APOE expression visualizations...\n")
  
  # 1. APOE expression by sample (like genotype frequency plot)
  if ("APOE" %in% all_apoe_stats$gene) {
    apoe_data <- all_apoe_stats[all_apoe_stats$gene == "APOE", ]
    
    # Genotype-like frequency plot
    p1 <- ggplot(apoe_data, aes(x = sample, y = mean_expression, fill = apoe_group)) +
      geom_col() +
      geom_text(aes(label = sprintf("%.3f", mean_expression)), vjust = -0.5, size = 3) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      labs(title = "APOE Expression by Sample (Genotype-like Analysis)",
           x = "Sample", y = "Mean Expression", fill = "APOE Group")
    
    ggsave("figures/APOE_analysis/APOE_expression_by_sample_genotype_style.pdf", p1, width = 14, height = 8)
    
    # 2. APOE expression by group (like population analysis)
    p2 <- ggplot(apoe_data, aes(x = apoe_group, y = mean_expression, fill = apoe_group)) +
      geom_boxplot() +
    ...
    ggsave("figures/APOE_analysis/APOE_expression_by_genotype_group.pdf", p2, width = 10, height = 6)
    
    # 3. Percentage of cells expressing APOE (like carrier rate analysis)
    p3 <- ggplot(apoe_data, aes(x = apoe_group, y = percent_expressing, fill = apoe_group)) +
      geom_col() +
      geom_text(aes(label = sprintf("%.1f%%", percent_expressing)), vjust = -0.5) +
      theme_minimal() +
      labs(title = "APOE Expression Rate by IDH Genotype Group",
           x = "IDH Genotype Group", y = "Percentage of Cells Expressing APOE (%)", fill = "Group")
    
    ggsave("figures/APOE_analysis/APOE_expression_rate_by_genotype.pdf", p3, width = 10, height = 6)
    
    # 4. APOE expression by diagnosis
    p4 <- ggplot(apoe_data, aes(x = diagnosis, y = mean_expression, fill = diagnosis)) +
      geom_boxplot() +
      geom_jitter(width = 0.2, alpha = 0.6) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    ...
    ggsave("figures/APOE_analysis/APOE_expression_by_diagnosis.pdf", p4, width = 12, height = 6)
  }
...
# Cell-level analysis (similar to haplotype analysis)
cat("\n=== CELL-LEVEL APOE ANALYSIS ===\n")
...
analyze_apoe_cell_level <- function(data, sample_name) {
  ...
  # Create cell-level data (similar to haplotype data)
  cell_data <- data.frame(
    sample = sample_name,
    cell_id = data$cells,
    apoe_expression = apoe_expr,
    apoe_detected = apoe_expr > 0,
    apoe_level = case_when(
      apoe_expr == 0 ~ "None",
      apoe_expr <= 1 ~ "Low",
      apoe_expr <= 5 ~ "Medium",
      TRUE ~ "High"
...
if (length(cell_level_results) > 0) {
  ...
  all_cell_data$apoe_group <- sample_meta$apoe_group
  ...
  # 1. APOE expression level distribution (like haplotype frequency)
  level_freq <- all_cell_data %>%
    filter(apoe_detected) %>%
    count(apoe_level) %>%
    mutate(freq = n/sum(n))
  
  p1 <- ggplot(level_freq, aes(x = apoe_level, y = freq, fill = apoe_level)) +
    geom_col() +
    geom_text(aes(label = sprintf("%.1f%%", freq * 100)), vjust = -0.5) +
    theme_minimal() +
    labs(title = "APOE Expression Level Distribution (Expressing Cells)",
         x = "APOE Expression Level", y = "Frequency", fill = "Level")
  
  ggsave("figures/APOE_analysis/APOE_expression_level_distribution.pdf", p1, width = 10, height = 6)
  
  # 2. APOE expression level by genotype group
  p2 <- ggplot(all_cell_data[all_cell_data$apoe_detected, ], 
               aes(x = apoe_group, fill = apoe_level)) +
    geom_bar(position = "fill") +
    theme_minimal() +
    labs(title = "APOE Expression Level Distribution by IDH Genotype",
         x = "IDH Genotype Group", y = "Proportion", fill = "APOE Level")
  
  ggsave("figures/APOE_analysis/APOE_level_by_genotype.pdf", p2, width = 10, height = 6)
  
  # 3. APOE expression vs number of genes (like correlation analysis)
  p3 <- ggplot(all_cell_data[all_cell_data$apoe_detected, ], 
               aes(x = n_genes, y = apoe_expression, color = apoe_group)) +
    geom_point(alpha = 0.6) +
    geom_smooth(method = "lm", se = TRUE) +
    theme_minimal() +
    labs(title = "APOE Expression vs Number of Genes",
         x = "Number of Genes", y = "APOE Expression", color = "IDH Genotype")
  
  ggsave("figures/APOE_analysis/APOE_vs_genes_scatter.pdf", p3, width = 10, height = 6)
  
  # 4. Summary statistics by group (like population summary)
  group_summary <- all_cell_data %>%
    ...
    summarise(
      ...
      high_expression_rate = mean(apoe_level == "High", na.rm = TRUE) * 100
    )
  
  write.csv(group_summary, "results/APOE_analysis/APOE_group_summary.csv", row.names = FALSE)
  
  p4 <- ggplot(group_summary, aes(x = apoe_group, y = percent_positive, fill = apoe_group)) +
    geom_col() +
    geom_text(aes(label = sprintf("%.1f%%", percent_positive)), vjust = -0.5) +
    theme_minimal() +
    labs(title = "APOE Expression Rate by IDH Genotype Group",
         x = "IDH Genotype Group", y = "Percentage of Cells Expressing APOE (%)", fill = "Group")
  
  ggsave("figures/APOE_analysis/APOE_expression_rate_summary.pdf", p4, width = 10, height = 6)
  
  cat("Cell-level analysis complete. Results saved to results/APOE_analysis/\n")
}

# Statistical analysis (similar to chi-square test)
...
if (length(apoe_results) > 0 && "APOE" %in% all_apoe_stats$gene) {
  ...
  if (length(unique(apoe_data$apoe_group)) > 1) {
    aov_result <- aov(mean_expression ~ apoe_group, data = apoe_data)
    cat("ANOVA results for APOE expression by IDH genotype group:\n")
  ...
  # Chi-square test for expression rate differences
  if (length(cell_level_results) > 0) {
    # Create contingency table for chi-square test
    cont_table <- table(all_cell_data$apoe_group, all_cell_data$apoe_detected)
    chisq_result <- chisq.test(cont_table)
    
    cat("\nChi-square test for APOE expression rate by IDH genotype group:\n")
    print(chisq_result)
    
    # Save chi-square results
    sink("results/APOE_analysis/APOE_chisq_results.txt", append = TRUE)
    cat("\nChi-square test for APOE expression rate by IDH genotype group:\n")
    print(chisq_result)
    sink()
  }
