# Direct Chromosome 19 APOE Analysis
# Fast navigation to specific genomic regions
...
# APOE genes on chromosome 19
apoe_genes <- c("APOE", "APOC1", "APOC2", "APOC3", "APOC4")

# Function to extract APOE genes directly from features
extract_apoe_features <- function(sample_dir) {
  ...
  tryCatch({
    ...
    # Find APOE genes
    apoe_indices <- which(gene_info[, "gene_name"] %in% apoe_genes)
    
    if (length(apoe_indices) > 0) {
      # Read matrix file and extract only APOE entries
      ...
      # Extract only APOE gene entries
      apoe_entries <- parsed_entries[parsed_entries[, 1] %in% apoe_indices, , drop = FALSE]
      
      # Calculate expression for each APOE gene
      apoe_results <- data.frame()
      for (i in seq_along(apoe_indices)) {
        gene_idx <- apoe_indices[i]
        gene_name <- gene_info[gene_idx, "gene_name"]
        
        # Get expression for this gene
        gene_entries <- apoe_entries[apoe_entries[, 1] == gene_idx, , drop = FALSE]
        ...
        apoe_results <- rbind(apoe_results, data.frame(
          sample = sample_name,
          gene = gene_name,
          ensembl_id = gene_info[gene_idx, "ensembl_id"],
...
cat("=== DIRECT CHROMOSOME 19 APOE ANALYSIS ===\n")
cat("Targeting APOE genes:", paste(apoe_genes, collapse = ", "), "\n")
cat("Analyzing", length(sample_dirs), "samples...\n")

# Extract APOE data from all samples
all_results <- lapply(sample_dirs, extract_apoe_features)
...
if (!is.null(all_results) && nrow(all_results) > 0) {
  ...
  # Save results
  write.csv(all_results, "results/APOE_analysis/chr19_APOE_results.csv", row.names = FALSE)
  
  # Create summary
  cat("\n=== CHROMOSOME 19 APOE SUMMARY ===\n")
  ...
  if (nrow(all_results) > 0) {
    ...
    p1 <- ggplot(all_results %>% filter(gene == "APOE"), 
      ...
      theme_minimal() +
      labs(title = "APOE Expression by IDH Genotype Group (Chr19)",
           x = "IDH Genotype Group", y = "Mean Expression", fill = "Group")
    
    ggsave("figures/APOE_analysis/chr19_APOE_expression.pdf", p1, width = 10, height = 6)
    ...
    p2 <- ggplot(all_results %>% filter(gene %in% apoe_genes), 
      ...
      geom_boxplot() +
      theme_minimal() +
      labs(title = "APOE Family Gene Expression (Chr19)",
           x = "Gene", y = "Mean Expression", fill = "Group")
    
    ggsave("figures/APOE_analysis/chr19_APOE_family.pdf", p2, width = 12, height = 6)
    ...
    p3 <- ggplot(all_results %>% filter(gene == "APOE"), 
      ...
      geom_jitter(width = 0.2, alpha = 0.6) +
      theme_minimal() +
      labs(title = "APOE Expression Percentage (Chr19)",
           x = "IDH Genotype Group", y = "Percentage of Expressing Cells", fill = "Group")
    
    ggsave("figures/APOE_analysis/chr19_APOE_percentage.pdf", p3, width = 10, height = 6)
  }
  
  cat("\nResults saved to: results/APOE_analysis/chr19_APOE_results.csv\n")
...
cat("\n=== CHROMOSOME 19 APOE ANALYSIS COMPLETE ===\n")
