### TCGA BRCA Correlation Analysis: Offline Version (Using Downloaded Data)
...
run_brca_offline_analysis <- function(data_file = "brca_data.rds") {
  
  cat("=== TCGA BRCA Correlation Analysis (Offline Mode) ===\n\n")
  
  # Check if we have local data
  if (file.exists(data_file)) {
    cat("✓ Found local BRCA data file\n")
    brca_data <- load_local_data(data_file)
  } else {
    cat("✗ No local BRCA data found\n")
    cat("Please download TCGA-BRCA data and save as 'brca_data.rds'\n")
    return(NULL)
  }
  
  # Continue with analysis if data is available
  cat("Proceeding with BRCA analysis...\n\n")
  
  # Step 1: Filter for primary tumor samples
  if ("shortLetterCode" %in% colnames(colData(brca_data))) {
    brca_data <- brca_data[, colData(brca_data)$shortLetterCode == "TP"]
    cat("✓ Filtered for primary tumor samples:", ncol(brca_data), "samples\n")
  } else {
  ...
  if (is.null(gene_names)) {
    ...
    if ("gene_name" %in% colnames(rowData(brca_data))) {
      gene_names <- rowData(brca_data)$gene_name
    } else if ("gene_id" %in% colnames(rowData(brca_data))) {
      gene_names <- rowData(brca_data)$gene_id
    } else {
  ...
  # Step 7: Save heatmap
  out_path <- file.path(out_dir, "Spearman_Correlation_Heatmap_BRCA_All_Immune_Offline.png")
  ...
  pheatmap(cor_block,
           ...
           fontsize = 10,
           main = "Spearman Correlation: Pathway vs Full Immune Marker Set (TCGA-BRCA) - Offline Analysis")
  dev.off()
  
  # Step 8: Save correlation matrix
  cor_csv_path <- file.path(out_dir, "Spearman_Correlation_Matrix_BRCA_All_Immune_Offline.csv")
  write.csv(cor_block, file = cor_csv_path)
  
  # Step 9: Print summary
  cat("\n=== BRCA Analysis Complete ===\n")
...
result <- run_brca_offline_analysis()
